...
# Resume controller functions
async def update_user_resume(user_id: int, resume_json: str):
    """Update the resume_json field for a user."""
    db = SessionLocal()
    try:
        user = db.query(User).filter(User.id == user_id).first()
        if not user:
            return None
        user.resume_json = resume_json
        db.add(user)
        db.commit()
        db.refresh(user)
        return user
...
async def get_user_resume(user_id: int):
    """Get the resume_json for a user."""
    db = SessionLocal()
    try:
        user = db.query(User).filter(User.id == user_id).first()
        if not user:
            return None
        return user.resume_json
    finally:
        db.close()
